            ...
            <div class="card">
                <div class="card-body">
                    <div class="text-center">
                        ...
                        <span>رقم الاشعار : {{ $invoiceReturn->serial_num }}</span><br>
                        <span>رقم الفاتورة : {{ App\Models\CustomerInvoice::withTrashed()->where('id',$invoiceReturn->customer_invoice_id)->first()->customer_inv_num}}</span><br>
                        <span>{{ $settings->name }}</span><br>
                        <span>{{ $invoiceReturn->branch->name }}</span><br>
                        <span>{{ $invoiceReturn->branch->address }}</span><br>
                        <span>{{ $invoiceReturn->branch->phone }}</span><br>
                        {{-- <span>فاتورة بيع</span><br> --}}
                        {{-- <span style="text-decoration: underline">مبيعات نقدية</span><br>
                        <span>صيدلية دان الرعاية</span><br> --}}
                        <span> {{ $invoiceReturn->branch->costCenter->name }}</</span><br>
                        <span>رقم التليفون : {{ $invoiceReturn->branch->address }}</span><br>
                        {{-- <span>الرقم الضريبي  : {{ $invoiceReturn->branch->address }}</span><br> --}}
                        <hr>

                        <span>تاريخ الفاتورة : {{ \Carbon\Carbon::parse($invoiceReturn->customer_inv_date_time)->toDateString() }}</span><br>
                        <span>توقيت الفاتورة : {{ \Carbon\Carbon::parse($invoiceReturn->customer_inv_date_time)->toTimeString() }}</span><br>
                    ...
                    <div>
                        ...
                        <table class="table ">
                            ...
                            <tbody>
                                @foreach ($invoiceReturnItems as $item)
                                    <tr>
                                        @php
                            ...
                            <tr style="background: transparent">
                                ...
                                <td class="details">{{$invoiceReturn->discount_percentage * $invoiceReturn->total_before_discount}}</td>
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            @endphp
                            <div class="text-center">
                                {!! DNS1D::getBarcodeSVG($invoiceReturn->customer_inv_num, 'C39',0.75,44) !!}
                            </div>
