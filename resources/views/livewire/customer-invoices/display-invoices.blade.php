        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between">
                    <h4 class="card-title h4">{{trans('admin.customers_invoices_list')}}</h4>
                    <div>
                        @can('اضافة-فاتورة-عميل')
                        <a href="{{ route('customers.create_invoice') }}" id="newInvoice" class="text-white">
                            <button type="button" class="btn bg-gradient-cyan"  title="إضافة فاتورة">
                                <span style="font-weight: bolder; font-size:">{{trans('admin.create_customer_invoice')}}</span>
                            </button>
                        </a>
            ...
            <div class="card-body">
                    ...
                    <div class="d-flex justify-content-around">
                       ...
                       {{-- <div class="form-group w-50 mx-1">
                            <label for="from_date">بحث بباركود الفاتورة</label>
                            <input type="text" class="form-control ml-2"  wire:model.live="invoice_barcode" placeholder="باركود الفاتورة">
                        </div> --}}
                    </div>
                        ...
                        <tbody>
                            @foreach ($customerInvoices as $customerInv)
                                ...
                                <td>{{ $customerInv->customer_inv_num }}</td>
                                    ...
                                    @endphp
                                    <td>
                                        <a href="{{ route('customers.cash_pay_invoice',[ 'inv_num' => $customerInv->customer_inv_num ]) }}">
                                            <button type="button" {{ $selectedTranaction ? 'disabled' :'' }}
                                     ...
                                     <td>
                                        <a href="{{ route('customers.print_invoice',[ 'inv_num' => $customerInv->customer_inv_num ]) }}">
                                    ...
                                    <td>
                                        <a href="#" class="modal-effect"
                                            ...
                                            <button class="btn " {{ $customerInv->return_status == 10  || $customerInv->return_status == 12 ? 'disabled' : ''}}>
                                    ...
                                    <td>
                                        ...
                                        @elseif($customerInv->return_status == 12)
                                            فاتورة محذوفة
                                        @endif
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ route('customers.create_debit_note',['inv_num' => $customerInv->customer_inv_num]) }}">
                                            <button type="button" title="إصدار إشعار مدين" {{$customerInv->return_status == 10 || $customerInv->return_status == 12 ? 'disabled' : ''}}   class="btn btn-outline-success btn-sm mx-1" title="{{trans('admin.edit')}}">
                                                <i class="far fa-edit"></i>
                                            </button>
                                        </a>
                                    </td>
                                    <td>
                                        <a href="{{ route('customers.create_invoice_return',['inv_num' => $customerInv->customer_inv_num]) }}">
                                            <button type="button" title="إصدار إشعار دائن" {{$customerInv->return_status == 10 || $customerInv->return_status == 12 ? 'disabled' : ''}}   class="btn btn-outline-info btn-sm mx-1" title="{{trans('admin.edit')}}">
                                                <i class="far fa-edit"></i>
                                            </button>
                                        </a>
                                        {{-- <button type="button"   class="btn btn-outline-info btn-sm mx-1" "
                                            data-toggle="modal"
                                            wire:click="$dispatch('updatecustomerInvoice',{id:{{$customerInv->id}}})">
                                        </button> --}}
                                    </td>
                                    @endcan
                                    {{-- @can('حذف-فاتورة-عميل')
                                    <td>
                                        <button type="button" class="btn btn-outline-danger btn-sm mx-1" {{$customerInv->return_status == 10  || $customerInv->return_status == 12 ? 'disabled' : ''}}  title="{{trans('admin.delete')}}"
                                            data-toggle="modal"
                                            wire:click="$dispatch('deleteCustomerInvoice',{id:{{$customerInv->id}}})">
