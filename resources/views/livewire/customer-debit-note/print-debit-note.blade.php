            ...
            <div class="card">
                <div class="card-body">
                    <div class="text-center">
                        <h6>إشعار مدين للفاتورة الضريبية المبسطة</h6><br>
                        <span>رقم الاشعار : {{ $invoiceDebitNote->serial_num }}</span><br>
                        <span>رقم الفاتورة : {{ App\Models\CustomerInvoice::withTrashed()->where('id',$invoiceDebitNote->customer_invoice_id)->first()->customer_inv_num}}</span><br>
                        <span>{{ $settings->name }}</span><br>
                        <span>{{ $invoiceDebitNote->branch->name }}</span><br>
                        <span>{{ $invoiceDebitNote->branch->address }}</span><br>
                        <span>{{ $invoiceDebitNote->branch->phone }}</span><br>
                        {{-- <span>فاتورة بيع</span><br> --}}
                        {{-- <span style="text-decoration: underline">مبيعات نقدية</span><br>
                        <span>صيدلية دان الرعاية</span><br> --}}
                        <span> {{ $invoiceDebitNote->branch->costCenter->name }}</</span><br>
                        <span>رقم التليفون : {{ $invoiceDebitNote->branch->address }}</span><br>
                        {{-- <span>الرقم الضريبي  : {{ $invoiceDebitNote->branch->address }}</span><br> --}}
                        <hr>

                        <span>تاريخ الفاتورة : {{ \Carbon\Carbon::parse($invoiceDebitNote->customer_inv_date_time)->toDateString() }}</span><br>
                        <span>توقيت الفاتورة : {{ \Carbon\Carbon::parse($invoiceDebitNote->customer_inv_date_time)->toTimeString() }}</span><br>
                    ...
                    <div>
                        @if($invoiceDebitNoteItems->count() > 0)
                        <style>
                            .table thead tr th{
                                text-align:center;
                            }
                        ...
                        <table class="table ">
                            ...
                            <tbody>
                                @foreach ($invoiceDebitNoteItems as $item)
                                    <tr>
                                        @php
                            ...
                            <tr style="background: transparent">
                                ...
                                <td class="details">{{$invoiceDebitNote->discount_percentage * $invoiceDebitNote->total_before_discount}}</td>
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            @php
                                ...
                                $fullDomain = $domainWithLang ."/invoices/print/". $invoiceDebitNote->customer_inv_num;
                            @endphp
                            <div class="text-center">
                                {!! DNS1D::getBarcodeSVG($invoiceDebitNote->customer_inv_num, 'C39',0.75,44) !!}
                            </div>
