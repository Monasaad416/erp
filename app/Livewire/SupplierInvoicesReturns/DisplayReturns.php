...
class DisplayReturns extends Component
{
    ...
    public function render()
    {
        if(Auth::user()->roles_name == 'سوبر-ادمن') {
            $items = SupplierReturn::where( function($query) {
            ...
            $items = SupplierReturn::where('branch_id',Auth::user()->branch_id)->where( function($query) {
        ...
        return view('livewire.supplier-invoices-returns.display-returns',[
            'items' => $items
        ]);
        }
