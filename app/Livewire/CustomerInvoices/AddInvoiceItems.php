...
class AddInvoiceItems extends Component
{
    ...
    public $invoice,$supp_inv_num,$customers,$units,$product, $product_id,$product_name_ar,$product_name_en, $product_code, $unit,
    ...
    ,$wholesale_inc_vat,$payment_type,$status,$supp_inv_date_time,$is_approved,$is_pending=0,$notes,$customer_id,$item_discount_percentage=0;


    public function mount($invoice){
        $this->invoice = $invoice ;
        $this->invoice_products = CustomerInvoiceItem::where('customer_invoice_id',$invoice->id)->latest()->get();
    ...
    public static function getNextInvoiceNumber()
    {
        ...
        $currentInvoiceNumber = CustomerInvoice::whereYear('created_at',$year)->max('supp_inv_num');
    ...
    {
        $currentSerial = CustomerInvoice::max('serial_num');
        if($currentSerial) {
    ...
    public function create() {
        ...
        // try{
         ...
         if(in_array($this->product_id,$ids) ){
            foreach($this->invoice_products as $item ){
                ...
                $invoice = CustomerInvoiceItem::create([
        ...
        $latestInvoiceItem  = CustomerInvoiceItem::orderBy('updated_at','DESC')->latest()->first();
          // dd($latestInvoiceItem);


        $inventory= Inventory::where('product_id', $latestInvoiceItem->product_id)->where('branch_id',1)->latest()->first();
        ...
        Inventory::create([
            ...
            'latest_sale_price' => $inventory->latest_sale_price ,
            'in_qty' => 0,
            'out_qty' => $item->qty,
        ...
        $latestInvoiceItem->inventory_balance = $latestInvoiceItem->qty + $inventoryQty;
        $latestInvoiceItem->save();
        ...
        $this->dispatch(
        'alert',
            text: trans('admin.customer_invoice_Item_created_successfully'),
            icon: 'success',
            confirmButtonText: trans('admin.done')
        );

            // return redirect()->route('customers.create_invoice_items',['invoice_num' => $this->invoice->supp_inv_num]) ;
    ...
    {
        return view('livewire.customer-invoices.add-invoice-items');
    }
}
