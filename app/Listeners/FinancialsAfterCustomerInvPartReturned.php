...
class FinancialsAfterCustomerInvPartReturned
{
    /**
    ...
    public function handle(object $event): void
    {
        ...
        $invoiceReturn = $event->invoiceReturn ;
        $item = $event->item;
        $tax = $event->tax ;
        $price = $event->price ;
        //dd($invoiceReturn);
        ...
        $treasury = Treasury::select('id','name_'.LaravelLocalization::getCurrentLocale().' as name','account_num')->where('branch_id',$invoiceReturn->branch_id)->first();
        ...
        $debitAccountParent1 = Account::where('account_num',42)->first();//مردودات المبيعات
       // dd($debitAccountParent1);
        //مردودات ومسموحات مبيعات الفرع
        ...
        $entry1->branch_id = $invoiceReturn->branch_id;
        $entry1->jounralable_type = 'App\Models\CustomerReturn';
        $entry1->jounralable_id = $item->id;
        ...
        $entry2->branch_id = $invoiceReturn->branch_id;
        $entry2->jounralable_type = 'App\Models\CustomerReturn';
        $entry2->jounralable_id = $item->id;
        ...
        $entry2->save();
            ...
            //dd($product,$item->return_qty);
            $purchasePrices = Inventory::where('product_id',$product->id)->latest()->first()->latest_purchase_price * $item->return_qty;
            ...
            $entry3->branch_id = $invoiceReturn->branch_id;
            $entry3->jounralable_type = 'App\Models\CustomerReturn';
            $entry3->jounralable_id = $item->id;
