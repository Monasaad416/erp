...
class FinancialsAfterSuppItemReturned
{
    /**
    ...
    public function handle(object $event): void
   {
        ...
        if($invoice->return_payment_type == 'cash') {
            ...
            if($invoice->payment_type == 'cash' || $invoice->payment_type == 'by_check'){
                ...
                $entry1->debit_amount = $totalReturnPrice * (1-$settings->vat);
                $entry1->credit_amount = $totalReturnPrice * (1-$settings->vat);
                ...
                $entry2->debit_amount = $totalReturnPrice * $settings->vat;
                $entry2->credit_amount = $totalReturnPrice * $settings->vat;
        ...
        //نهاية تحديث ارصدة حسابات الاباء بالشجرة المحاسبية
            ...
            $debitAccount = Supplier::select('id',
          ...
          //بداية تحديث ارصدة حسابات الاباء بالشجرة المحاسبية
            ...
            //dd($creditLevels1);
            foreach ($creditLevels1 as $relatedCreditLevel2) {

                $levelCreditAccount1 = Account::where('account_num',$relatedCreditLevel2)->first();
                //dd($levelCreditAccount1->current_balance);
                $levelCreditAccount1->update([
